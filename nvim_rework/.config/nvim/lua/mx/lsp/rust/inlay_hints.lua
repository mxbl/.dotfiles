-- My inlay hints module, mostly copied from simrat39/rust-tools.nvim
...
function M.enable_cache_autocmd()
    vim.cmd(string.format(
        [[
            ...
            autocmd BufWritePost,BufReadPost,BufEnter,BufWinEnter,TabEnter,TextChanged *.rs :lua require"mx.lsp.rust".inlay_hints.cache()
            %s
            augroup END
        ]],
        opts.only_current_line
        and
            "autocmd CursorMoved,CursorMovedI *.rs :lua require'mx.lsp.rust'.inlay_hints.render()"
        or
            ""
    ))
end
...
local function render_line(line, line_hints, bufnr)
    ...
    -- show other hints with a comma and a thicc arrow
